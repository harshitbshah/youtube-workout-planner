...
| `components/PWAInstallPrompt.tsx` | Listens for `beforeinstallprompt`; shows bottom install banner; sessionStorage-dismissed on "Not now" |
| `app/offline/page.tsx` | Offline fallback shown by service worker when navigation fails with no cached response |
| `public/manifest.json` | PWA manifest: standalone display, start_url=/dashboard, icons, theme_color |
| `public/icons/icon.svg` | Source SVG for app icon (geometric "P" monogram on zinc-900) |
| `public/icons/icon-{192,512}.png` | Rasterised PWA icons (generated by scripts/generate-icons.mjs) |
| `scripts/generate-icons.mjs` | Node script: reads icon.svg, uses sharp to write 192/512 PNG + maskable variants |
